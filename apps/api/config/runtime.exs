...
#
#     PHX_SERVER=true bin/cartograph_backend start
#
# Alternatively, you can use `mix phx.gen.release` to generate a `bin/server`
# script that automatically sets the env var above.
if System.get_env("PHX_SERVER") do
  config :cartograph_backend, CartographBackendWeb.Endpoint, server: true
end
...
if config_env() == :prod do
  ...
  # (unset / anything else)   → single-node, no clustering
  ...
  # Job data sandbox (Files area + file DSL steps). In a release the cwd is
  # not the source tree, so the `data` default would land somewhere volatile —
  # require an explicit persistent path in prod.
  step_data_root =
    System.get_env("STEP_DATA_ROOT") ||
      raise """
      environment variable STEP_DATA_ROOT is missing.
      Set it to a persistent directory for job data, e.g. /var/lib/cartograph/data
      """

  config :cartograph_backend, :step_data_root, step_data_root
