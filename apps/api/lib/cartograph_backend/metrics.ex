defmodule CartographBackend.Metrics do
  ...
  def dashboard_metrics do
    ...
    running        = Repo.aggregate(from(e in TaskExecution, where: e.status in ^Status.active()), :count)

    statuses = Repo.all(from e in TaskExecution,
      where: e.status in ^Status.terminal(),
      select: e.status)

    total_terminal = length(statuses)
    successes      = Enum.count(statuses, &(&1 == Status.success()))
    success_rate   = if total_terminal > 0, do: Float.round(successes / total_terminal * 100, 1), else: nil
