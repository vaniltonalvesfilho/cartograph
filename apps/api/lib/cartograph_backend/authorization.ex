defmodule CartographBackend.Authorization do
  ...
  """
  def authorize_create_group(user, parent_group_id),
    do: require_parent_create(user, Group, parent_group_id)

  def authorize_create_project(user, group_id), do: require_parent_create(user, Group, group_id)

  def authorize_create_task(user, project_id),
    do: require_parent_create(user, Project, project_id)
  ...
  def scope(%{id: uid}) do
    ...
    nav_groups =
      (Enum.flat_map(full_groups, fn gid -> ancestors(Map.get(parent_of, gid), parent_of) end) ++
         Enum.flat_map(full_projects, fn pid ->
           ancestors(Map.get(group_of_project, pid), parent_of)
         end) ++
