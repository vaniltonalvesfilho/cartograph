defmodule CartographBackend.DataSources.DataSource do
  ...
  def changeset(ds, attrs) do
    ds
    |> cast(attrs, [:name, :slug, :adapter, :host, :port, :database_name, :username, :password, :ssl, :notes])
    ...
    |> validate_format(:slug, ~r/^[a-z0-9]+(-[a-z0-9]+)*$/, message: "must be lowercase slug (e.g. mysql-local)")
