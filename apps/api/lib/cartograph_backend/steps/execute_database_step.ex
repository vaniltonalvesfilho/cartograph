defmodule CartographBackend.Steps.ExecuteDatabaseStep do
    ...
    with {:slug,   true}      <- {:slug,   is_binary(slug) and slug != ""},
      ...
      case run_execute(ds, password, query, rows_list) do
        {:ok, count} ->
          ...
          {:ok, ctx}
        {:error, reason} -> {:error, "executeDatabase: #{reason}"}
      end
    else
      {:slug,   false}       -> {:error, "executeDatabase: 'source' param is required"}
      {:query,  false}       -> {:error, "executeDatabase: 'query' param is required"}
      {:ds,     {:error, _}} -> {:error, "executeDatabase: data source '#{slug}' not found"}
      {:access, false}       -> {:error, "executeDatabase: data source '#{slug}' is not accessible from this project"}
    end
  end
  ...
  defp exec_postgres(ds, password, query, rows_list) do
    opts = [hostname: ds.host, port: ds.port, database: ds.database_name,
